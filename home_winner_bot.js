...
const token = '893572140:AAHKUbW2J_vZrTuBnfrBV1ECPzkHn6wkeKU';
const bot = new TelegramBot(token, {polling: true});

const channel = '-1001184173411';
      ...
      _.forEach(filteredResults, function(item) {
        ...
        rp('https://api.betsapi.com/v1/event/view?token=8334-BCLtMmtKT698vk&event_id=' + item.id)
          .then(function (response2) {
            ...
            if (dangerAttacksDiff >= 3 && goalsOnTargetDiff >= 0) {
              rp('https://api.betsapi.com/v1/event/odds?token=8334-BCLtMmtKT698vk&event_id=' + item.id)
                .then(function (response3) {
                  ...
                  let oddsKef = parseFloat(currentResultOdd.home_od)/parseFloat(resultOdd.home_od)

                  let handicapArray = odd.handicap.split(',');

                  if (parseFloat(resultOdd.home_od) >= 3.5 && oddsKef >= 0.94) {
