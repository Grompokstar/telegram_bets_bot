  ...
  rp('https://api.betsapi.com/v2/events/inplay?sport_id=1&token=8334-BCLtMmtKT698vk')
    .then(function (response) {
      ...
      filteredResults = _.filter(results, function(item) {
        ...
        let totalGoals = parseInt(item.scores['2'].away) - parseInt(item.scores['2'].home);
      ...
      _.forEach(filteredResults, function(item) {
        ...
        rp('https://api.betsapi.com/v1/event/view?token=8334-BCLtMmtKT698vk&event_id=' + item.id)
          .then(function (response2) {
            ...
            let goalsOffTargetDiff = parseInt(view.stats.off_target[0]) - parseInt(view.stats.off_target[1]);
            ...
            let attacksRatioKefHome = parseInt(view.stats.attacks[0])/parseInt(view.stats.attacks[1]);


            if (advantageTeam === 'home' && dangerAttacksSumm >= 18 && dangerAttacksDiff >= 5 && attacksSumm >= 33
              ...
              rp('https://api.betsapi.com/v1/event/odds?token=8334-BCLtMmtKT698vk&event_id=' + item.id)
                .then(function (response3) {
                  ...
                  if (odd && (parseFloat(odd.over_od <= 1.5 && handicapArray[0]) <= 2.5
                    || parseFloat(odd.over_od) < 1.85 && parseInt(handicapArray[0]) === 3
                    || parseFloat(odd.over_od) < 2 && parseFloat(handicapArray[0]) > 3 )
                    && currentResultOdd && (parseFloat(currentResultOdd.home_od) >= 1.1 && parseFloat(currentResultOdd.home_od) <= 1.9)) {
